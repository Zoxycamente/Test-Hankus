...
class OptionsMenuState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		Application.current.window.title = "Friday Night Funkin': V.S Hankus 2023 Remake - Options";
		...
		categoryMap = [
			...
			'appearance' => [
				[
					...
					['Healthbar Overlay', getFromOption],
					['Overlay', getFromOption],
					...
					['Classic Health Bar', getFromOption],
					["Show Canon text", getFromOption],
		...
		centerMark = new FlxText(0,0, 0, '- Fnf Hankus leaked build feat. your MOM${(Init.trueSettings.get('Show Canon text') ? " [Canon]": "")} -');
		centerMark.alpha = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		switch (activeSubgroup.members[curSelection].text) {
			case 'Show Canon text':
				centerMark.text = '- Fnf Hankus leaked build feat. your MOM${(Init.trueSettings.get('Show Canon text') ? " [Canon]": "")} -';
				centerMark.setFormat(Paths.font('vcr.ttf'), 24, FlxColor.WHITE);
				centerMark.setBorderStyle(OUTLINE, FlxColor.BLACK, 2);
				centerMark.screenCenter(X);
				centerMark.y = (FlxG.height / 24) - 10;
			...
			case 'Display Accuracy':
				...
				centerMark.text = 'Score: 0 ${(Init.trueSettings.get('Display Accuracy') ? ' Misses: 0 • Accuracy: 0%' : '')}';
				centerMark.alpha = 1;
