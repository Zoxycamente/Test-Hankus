...
class FreeplayState extends MusicBeatState
{
	...
	// load in all songs that exist in folder
		...
		Application.current.window.title = "Friday Night Funkin': V.S Hankus 2023 Remake - Freeplay";

		// LOAD CHARACTERS
		bg = new FlxSprite().loadGraphic(Paths.image('menus/base/menuDesat'));
		add(bg);

		var grid:FlxBackdrop = new FlxBackdrop(FlxGridOverlay.createGrid(80, 80, 160, 160, true, 0x3300C3FF, 0x0));
